...
if (isset($_GET['acao']) && $_GET['acao'] == 'apagar') {
    ...
    $qr = 'DELETE FROM lc_movimento WHERE id=:id';

    $row = $PDO->prepare($qr);
    $row->bindParam(":id", $id, PDO::PARAM_STR);
    $row->execute();
...
if (isset($_POST['acao']) && $_POST['acao'] == 'editar_cat') {
    ...
    $qr = 'UPDATE lc_cat SET nome=:nome WHERE id=:id';

    $row = $PDO->prepare($qr);
    $row->bindParam(":nome", $nome, PDO::PARAM_STR);
    $row->bindParam(":id", $id, PDO::PARAM_STR);
    $row->execute();
...
if (isset($_GET['acao']) && $_GET['acao'] == 'apagar_cat') {
    ...
    $qr = 'SELECT c.id FROM lc_movimento m, lc_cat c WHERE c.id=m.cat && c.id=:id';
    $row = $PDO->prepare($qr);
    $row->bindParam(":id", $id, PDO::PARAM_STR);
    $row->execute();
    $result = $row->fetch();
    ...
    $qr = 'DELETE FROM lc_cat WHERE id=:id';
...
if (isset($_POST['acao']) && $_POST['acao'] == 'editar_mov') {
    ...
    $qr = 'UPDATE lc_movimento SET dia=:dia, tipo=:tipo, cat=:cat, descricao=:descricao, valor=:valor WHERE id=:id';
    
    $row = $PDO->prepare($qr);
    $row->bindParam(":id", $id, PDO::PARAM_STR);
    $row->bindParam(":dia", $dia, PDO::PARAM_STR);
    $row->bindParam(":tipo", $tipo, PDO::PARAM_STR);
    $row->bindParam(":cat", $cat, PDO::PARAM_STR);
    $row->bindParam(":descricao", $descricao, PDO::PARAM_STR);
    $row->bindParam(":valor", $valor, PDO::PARAM_STR);
    $row->execute();
...
if (isset($_POST['acao']) && $_POST['acao'] == 2) {
    ...
    $nome = $_POST['nome'];
    $qr = 'INSERT INTO lc_cat (nome) VALUES (:nome)';
...
if (isset($_POST['acao']) && $_POST['acao'] == 1) {
    ...
    $qr = 'INSERT INTO lc_movimento (dia,mes,ano,tipo,descricao,valor,cat) values (:dia,:mes,:ano,:tipo,:descricao,:valor,:cat)';

    $row = $PDO->prepare($qr);
    $row->bindParam(":dia", $dia, PDO::PARAM_STR);
    $row->bindParam(":mes", $mes, PDO::PARAM_STR);
    $row->bindParam(":ano", $ano, PDO::PARAM_STR);
    $row->bindParam(":tipo", $tipo, PDO::PARAM_STR);
    $row->bindParam(":descricao", $descricao, PDO::PARAM_STR);
    $row->bindParam(":valor", $valor, PDO::PARAM_STR);
    $row->bindParam(":cat", $cat, PDO::PARAM_STR);
...
<a href="../view/index.php">Usuários</a>
<a href="../relato/index.php">Relatório</a>
<a href="../relato/usuarios_pdf.php">Teste</a>
<a href="../tesouraria/relatorio.php">Teste2</a>
...
<tr>
<td colspan="2">
<?php echo '<h2><a target="_blank" href="../relato/relatorio_mensal.php">Salvar Relatorio PDF do Mes: </a></h2>'?> 
...
<?php
$qr = $PDO->query('SELECT id, nome FROM lc_cat');
...
<?php echo $row['nome']?>  
                     ...
                     <a style="font-size:12px; color:#666" onclick="return confirm('Tem certeza que deseja remover esta categoria?\nAtenção: Apenas categorias sem movimentos associados poderão ser removidas.')" href="?mes=<?php echo $mes_hoje?>&ano=<?php echo $ano_hoje?>&acao=apagar_cat&id=<?php echo $row['id']?>" title="Remover">[remover]</a>
                     <a href="javascript:;" style="font-size:12px; color:#666" onclick="document.getElementById('editar_cat_<?php echo $row['id']?>').style.display=''; document.getElementById('editar2_cat_<?php echo $row['id']?>').style.display='none'" title="Editar">[editar]</a>
...
<?php
$qr = $PDO->query("SELECT * FROM lc_cat");
...
$qr = $PDO->query("SELECT SUM(valor) as total FROM lc_movimento WHERE tipo=1 && mes='$mes_hoje' && ano='$ano_hoje'");

$row = $qr->fetch(PDO::FETCH_ASSOC);

$entradas=$row['total'];

$qr = $PDO->query("SELECT SUM(valor) as total FROM lc_movimento WHERE tipo=0 && mes='$mes_hoje' && ano='$ano_hoje'");
$row = $qr->fetch(PDO::FETCH_ASSOC);
...
?>
    ...
    <fieldset>
        <legend><strong>Entradas e Saidas deste mes</strong></legend>
        <table cellpadding="0" cellspacing="0" width="100%">
            ...
            <tr>
                <td><span style="font-size:18px; color:#C00">Saidas:</span></td>
...
$qr = $PDO->query("SELECT SUM(valor) as total FROM lc_movimento WHERE tipo=1 ");
$row = $qr->fetch(PDO::FETCH_ASSOC);

$entradas=$row['total'];

$qr = $PDO->query("SELECT SUM(valor) as total FROM lc_movimento WHERE tipo=0 ");
$row = $qr->fetch(PDO::FETCH_ASSOC);
...
<?php
$qr = $PDO->query("SELECT DISTINCT c.id, c.nome FROM lc_cat c, lc_movimento m WHERE m.cat=c.id && m.mes='$mes_hoje' && m.ano='$ano_hoje'");
...
if (isset($_GET['filtro_cat'])){
	if ($_GET['filtro_cat']!=''){	
		$filtros="&& cat='".$_GET['filtro_cat']."'";
                ...
                $qr = $PDO->query("SELECT SUM(valor) as total FROM lc_movimento WHERE tipo=1 && mes='$mes_hoje' && ano='$ano_hoje' $filtros");
                $row = $qr->fetch(PDO::FETCH_ASSOC);

                $entradas=$row['total'];

                $qr = $PDO->query("SELECT SUM(valor) as total FROM lc_movimento WHERE tipo=0 && mes='$mes_hoje' && ano='$ano_hoje' $filtros");
                $row = $qr->fetch(PDO::FETCH_ASSOC);
...
$qr = $PDO->query("SELECT * FROM lc_movimento WHERE mes='$mes_hoje' && ano='$ano_hoje' $filtros ORDER By dia");
...
$cat=$row['cat'];
$qr2 = $PDO->query("SELECT nome FROM lc_cat WHERE id='$cat'");

$row2 = $qr2->fetch(PDO::FETCH_ASSOC);
$categoria=$row2['nome'];
...
<td><?php echo $row['descricao']?> <em>(<a href="?mes=<?php echo $mes_hoje?>&ano=<?php echo $ano_hoje?>&filtro_cat=<?php echo $cat?>"><?php echo $categoria?></a>)</em> <a href="javascript:;" style="font-size:12px; color:#666" onclick="document.getElementById('editar_mov_<?php echo $row['id']?>').style.display='';  " title="Editar">[editar]</a></td>
<td align="right"><strong style="color:<?php if ($row['tipo']==0) echo "#C00"; else echo "#030"?>"><?php if ($row['tipo']==0) echo "-"; else echo "+"?><?php echo formata_dinheiro($row['valor'])?></strong></td>
    ...
    <tr style="display:none; background-color:<?php if ($cont%2==0) echo "#F1F1F1"; else echo "#E0E0E0"?>" id="editar_mov_<?php echo $row['id']?>">
        <td colspan="3">
            ...
            <b>Dia:</b> <input disabled type="text" name="dia" size="3" maxlength="2" value="<?php echo $row['dia']?>" />&nbsp;|&nbsp;
            ...
            <select name="cat">
<?php
$qr2 = $PDO->query("SELECT * FROM lc_cat");

while ($row2 = $qr2->fetch(PDO::FETCH_ASSOC)){
?>
...
<td align="right">
<hr size="1" />
<em>Tesouraria <strong></strong> - Desenvolvido por <a href="#">PHBS</a>. Versao 1.0.0 <a href="../controller/logoff.php">Sair do Sistema</a></em>
</td>
</tr>
</table>
</body>
