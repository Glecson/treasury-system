...
<body>
	<div class='container box-mensagem-crud'>
	    ...
	    if ($acao != 'excluir'):
			...
			if ($senha == ''):
				$mensagem .= '<li>Escolha uma Senha.</li>';
			endif;

			if ($cargo == ''):
				$mensagem .= '<li>Escolha o Cargo.</li>';
			endif;
		...
		if ($acao == 'incluir'):
			...
			$nome_foto = '../view/fotos/padrao.jpg';
			if(isset($_FILES['foto']) && $_FILES['foto']['size'] > 0):  
			    ...
			    $exploded = explode('.',$_FILES['foto']['name']);
 				$extensao=strtolower(end($exploded));
			    ...
			    if (array_search($extensao, $extensoes_aceitas) === false):
			     ...
			     if(is_uploaded_file($_FILES['foto']['tmp_name'])):  
			          ...
			          if (!move_uploaded_file($_FILES['foto']['tmp_name'], '../view/fotos/'.$nome_foto)):  
			               echo "Houve um erro ao gravar arquivo na pasta de destino!";  
			          endif;  
			     endif;  
			endif;

			$sql = 'INSERT INTO usuario (nome, email, senha, cargo, cpf, data_nascimento, telefone, celular, status, foto)
							   VALUES(:nome, :email, :senha, :cargo, :cpf, :data_nascimento, :telefone, :celular, :status, :foto)';
			...
			echo "<meta http-equiv=refresh content='3;URL=../view/index.php'>";
		endif;
		...
		if ($acao == 'editar'):
			...
			$sql = 'UPDATE usuario SET nome=:nome, email=:email, senha=:senha, cargo=:cargo, cpf=:cpf, data_nascimento=:data_nascimento, telefone=:telefone, celular=:celular, status=:status, foto=:foto ';
			$sql .= 'WHERE id = :id';
		...
		if ($acao == 'excluir'):
			...
			$sql = "SELECT foto FROM usuario WHERE id = :id AND foto <> 'padrao.jpg'";
			$stm = $conexao->prepare($sql);
			$stm->bindValue(':id', $id);
			$stm->execute();
			...
			if (!empty($cliente) && file_exists('fotos/'.$cliente->foto)):
				unlink("../view/fotos/" . $cliente->foto);
			endif;
